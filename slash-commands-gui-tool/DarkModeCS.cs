...
namespace DarkModeForms
{
	...
	public class DarkModeCS
	{
		...
		{
			//Fix has back color button change to black problem. 
            if (control is Button ) {
				if(IsDarkMode && control.BackColor.Name != "ff202020" && control.BackColor.Name != "Control" && control.BackColor.Name != "ButtonFace") {
					control.ForeColor = Color.Black;
                    return;
				}
                if (!IsDarkMode && control.BackColor.Name != "ff373737") {
                    control.ForeColor = Color.Black;
                    return;
                }
			...
			{
				control.GetType().GetProperty("LinkColor")?.SetValue(control, OScolors.TextActive);
	...
	public class MyRenderer : ToolStripProfessionalRenderer
	{
		...
		protected override void OnRenderItemText(ToolStripItemTextRenderEventArgs e)
		{
			if (e.Item.Enabled)
			{
				if(e.Text == "UI語言 [&L]" && e.TextColor.Name != "MenuText") {

				}
                if(e.TextColor.Name == "White" || e.TextColor.Name == "HighlightText" || e.TextColor.Name == "MenuText") e.TextColor = MyColors.TextActive;
